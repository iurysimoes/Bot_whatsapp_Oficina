...
const stageMontagem = async (numeroOS) => {
  let connection;
  ...
  try {
    ...
    if (result.rows[0].OSFP_DT_MONTAGEM_INI !== null) {
      await connection.execute(
        `UPDATE ORDEM_SERVICO_FUNI_PINT OS
                       SET OS.OSFP_DT_MONTAGEM_FIN = SYSDATE,
    ...
    } else if (result.rows[0].OSFP_DT_MONTAGEM_INI === null) {
      await connection.execute(
        `UPDATE ORDEM_SERVICO_FUNI_PINT OS
                        SET OS.OSFP_DT_MONTAGEM_INI = SYSDATE,
                            OS.OSFP_ESTAGIO     = 'MONTAGEM'
